...
export const vantagens = [
    ...
    { nome: 'Alcance', custo: 1, max: 2, descricao: 'Seus ataques acertam mais longe. 1pt: pode atingir alvos Longe sem penalidades, e Muito Longe com -1D (Perda). 2pt: pode atingir alvos Muito Longe sem penalidades.' },
    ...
    { 
      ...
      custo: 1, 
      repetivel: true, 
      descricao: 'Você é imune a algo que normalmente afeta outras criaturas.',
     ...
     { 
      ...
      custo: 1,
      repetivel: true,
      descricao: 'Você é treinado para enfrentar um certo tipo de criatura. Em todos os testes contra elas, você consegue acerto crítico com 5 ou 6. Custa 2pt para grupos muito comuns.',
    ...
    { nome: 'Inventário', custo: 1, max: 3, descricao: 'Você carrega itens consumíveis. 1pt: Pequeno (3 comuns, 1 incomum). 2pt: Grande (5 comuns, 2 incomuns). 3pt: Supremo (5 comuns, 4 incomuns, 1 raro).' },
    { nome: 'Invisível', custo: 1, max: 2, descricao: 'Gaste uma ação e 3 PM para ficar invisível. Você tem +1D (Ganho) para se esconder e ataques contra você têm -1D (Perda). 1pt: a invisibilidade é interrompida se você atacar ou sofrer dano. 2pt: é interrompida apenas se você sofrer dano.' },
    { nome: 'Irresistível', custo: 1, descricao: 'Seus poderes são mais difíceis de resistir. Quando um alvo faz um teste para evitar um de seus poderes, você pode gastar 2 PM para aumentar a meta do teste em +3.' },
    { nome: 'Maestria', custo: 1, repetivel: true, descricao: 'Escolha uma perícia que você tem. Em testes dessa perícia, pode gastar 1 PM para ter um acerto crítico com 5 ou 6.' },
    ...
    { nome: 'Patrono', custo: 1, descricao: 'Você serve a uma pessoa ou organização. Ao fazer um teste de compra para algo que ajude na missão, pode gastar 1 PM para receber +1D (Ganho). Também recebe um item consumível extra de cada raridade disponível em seu Inventário.' },
    { nome: 'Punição', custo: 1, max: 2, repetivel: true, descricao: 'Você pode impor um efeito negativo. Escolha uma desvantagem de mesmo valor. Faça um ataque e gaste 2 PM. Se vencer a defesa, o alvo sofre os efeitos da desvantagem escolhida.' },
    { nome: 'Regeneração', custo: 1, max: 2, descricao: 'Você se recupera rapidamente. 1pt: recupera 1 PV no início do seu turno. 2pt: recupera 3 PV no início do seu turno. Em testes de morte, seu resultado nunca é pior que Inconsciente.' },
    ...
    { nome: 'Riqueza', custo: 2, max: 6, descricao: 'Você tem recursos financeiros de escala superior. Em testes de compra, pode gastar PM para subir sua escala. 2pt: Sugoi (1 nível, 2 PM), 4pt: Kiodai (2 níveis, 4 PM), 6pt: Kami (3 níveis, 6 PM).' },
    { 
      nome: 'Sentido', 
      custo: 1, 
      repetivel: true, 
      descricao: 'Você tem um sentido melhor ou diferente que permite fazer testes de Percepção em situações impossíveis para outros.',
      opcoes: ['Aguçado', 'Infravisão', 'Intuição', 'Radar', 'Raio X']
    },
    { nome: 'Telepata', custo: 1, descricao: 'Você pode ler ou sentir os pensamentos de outros. Pode usar um movimento e gastar 1 PM para ter +1D (Ganho) em um teste de perícia social ou para prever os movimentos de um adversário em combate, ganhando +1D (Ganho) em defesa contra ele.' },
    { nome: 'Teleporte', custo: 1, descricao: 'Você pode usar um movimento e gastar PM para se deslocar até qualquer lugar que possa ver. 1 PM por passo de distância. Também pode gastar 3 PM para ter +1D (Ganho) de defesa contra um ataque.' },
    ...
    { nome: 'Transformação', custo: 1, max: 2, repetivel: true, descricao: 'Você pode mudar de forma e poderes. Crie outra ficha com o mesmo número de pontos. 1pt: os efeitos de todas as desvantagens são combinados. 2pt: cada forma sofre apenas os efeitos de suas próprias desvantagens.' },
...
export const desvantagens = [
    ...
    { nome: 'Assombrado', custo: -1, max: -2, descricao: 'Você é assombrado por algo que tira sua concentração. -1pt: Ao entrar em combate ou cena de ação, role 1D. Com resultado 1, todos os seus testes têm -1D (Perda) até o fim da cena. -2pt: A Perda ocorre com qualquer resultado ímpar.' },
    ...
    { nome: 'Aura', custo: -1, max: -2, descricao: 'Você emana uma aura pesada. -1pt: Qualquer teste de outras pessoas (aliados ou inimigos) realizado Perto de você tem -1D (Perda). -2pt: O efeito se estende para qualquer teste Longe de você ou menos.' },
    ...
    { nome: 'Diferente', custo: -1, descricao: 'Seu corpo é muito diferente de um corpo humanoide comum. Você tem -1D (Perda) ao usar armas, roupas e equipamentos feitos para outros.' },
    { nome: 'Elo Vital', custo: -1, descricao: 'Escolha um aliado que também deve ter esta desvantagem. Sempre que um de vocês sofre dano, o outro também perde PV na mesma quantidade. Curar um não cura o outro.' },
    ...
    { nome: 'Fraqueza', custo: -1, max: -2, descricao: 'Você é vulnerável a um objeto, elemento ou condição. Enquanto estiver Perto dessa coisa, todos os seus testes têm -1D (Perda). O custo depende de quão comum é a fonte da fraqueza (-1pt para incomum, -2pt para comum).' },
    { nome: 'Fúria', custo: -2, descricao: 'Sempre que você sofre dano ou fica irritado, deve fazer um teste de Resistência. Se falhar, você entra em frenesi. Durante a Fúria, todos os seus testes que não sejam de ataque (incluindo defesa) têm -1D (Perda) e suas vantagens gastam o dobro de PM.' },
    { nome: 'Inapto', custo: -1, repetivel: true, descricao: 'Escolha uma perícia que você não possui. Você é incompetente nela. Sempre que fizer um teste com essa perícia, você está em -1D (Perda) e qualquer falha é considerada uma falha crítica.' },
    ...
    { nome: 'Maldição', custo: -1, max: -2, descricao: 'Você é vítima de uma maldição que o perturba. -1pt: A maldição é suave, mais irritante e pode causar -1D (Perda) em algumas situações. -2pt: A maldição é muito grave e desafiadora, com efeitos definidos pelo mestre.' },
    ...
    { nome: 'Pacifista', custo: -1, max: -2, descricao: 'Você rejeita conflito. -1pt: Você não faz testes de ataque. -2pt: Você não faz nada que cause dano. Se violar seu pacifismo, você tem -1D (Perda) em todos os testes até se redimir.' },
    { nome: 'Pobreza', custo: -1, descricao: 'Você não tem recursos financeiros. Todos os seus testes de compra são feitos com -1D (Perda).' },
    { nome: 'Ponto Fraco', custo: -1, descricao: 'Você tem uma vulnerabilidade explorável. Um adversário que conheça seu Ponto Fraco pode gastar 1 PM para receber +1D (Ganho) contra você em um teste que explore esse problema.' },
    { nome: 'Protegido', custo: -1, descricao: 'Existe alguém que você precisa proteger. Sempre que o Protegido está desaparecido, indefeso ou ferido (com qualquer PV abaixo do total), todos os seus testes têm -1D (Perda).' },
    { nome: 'Restrição', custo: -1, max: -2, descricao: 'Certa condição torna mais difícil usar seus poderes, dobrando o custo em PM de suas vantagens. O custo depende de quão comum é a restrição (-1pt para incomum, -2pt para comum).' },
    { nome: 'Sem Vida', custo: -2, descricao: 'Você não é um ser vivo. Você não recupera PV de formas normais (descanso, itens, Cura). Apenas pode ser consertado com perícias como Máquinas ou Mística. Em testes de morte, seu resultado nunca é pior que Inconsciente.' },
    { nome: 'Tapado', custo: -1, descricao: 'Você não é muito brilhante. Em testes de Habilidade relacionados a inteligência e raciocínio, você tem -1D (Perda) e nunca consegue acertos críticos.' },
    {
      ...
      custo: -1,
      repetivel: true,
      descricao: 'Você lida com um transtorno mental que afeta seu comportamento.',
      opcoes: ['Cleptomania', 'Compulsão', 'Distração', 'Fantasia', 'Fobia', 'Megalomania', 'Mitomania', 'Paranoia']
    },
    { nome: 'Utensílio', custo: -1, max: -2, descricao: 'Você precisa de um item para usar seus poderes. -1pt: Sem o item, você não consegue acertos críticos com uma perícia. -2pt: Sem o item, você não pode usar vantagens ou técnicas que gastam PM.' },
];
