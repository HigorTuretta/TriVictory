...
// Sub-componente para organizar a seção de Arquétipo
...
export const CharacterSheet = ({ characterData, gameData, onUpdate, onExit, onAddCustomItem, onUpdateCustomItem, onDeleteCustomItem, goToArchetypeCreator }) => {
    ...
    const handleArchetypeChange = (e) => {
        ...
        setCharacter(prev => ({ ...prev, archetype: newArchetype, archetypeChoices: {}, advantages: advantagesList }));
    ...
    return (
        <SheetContainer $isDead={character.isDead}>
            ...
            <Header>
                <CharacterNameInput type="text" value={character.name} onChange={(e) => handleUpdate({ name: e.target.value })} placeholder="Nome do Personagem" disabled={character.isDead} />
                <PointTracker points={points} basePoints={character.basePoints || 12} />
            </Header>

            <HeaderPanel>
                <Section><AttributeDisplay attributes={character.attributes} resources={resources} currentResources={{ pv_current: character.pv_current, pm_current: character.pm_current, pa_current: character.pa_current }} onAttributeChange={handleAttributeChange} onResourceChange={handleResourceChange} isEditing={isEditing} isDead={character.isDead} /></Section>
            </HeaderPanel>

            <SheetLayoutGrid>
                <LeftColumn>
                    <Section><SectionTitle>Dinheiro</SectionTitle><MoneyTracker money={character.money || { amount: 0, type: { nome: 'Moedas de Ouro', sigla: 'MO' } }} onUpdate={(newMoney) => handleUpdate({ money: newMoney })} isEditing={isEditing} isDead={character.isDead} /></Section>
                    <Section><SectionTitle>Level e XP</SectionTitle><LevelXPTracker level={character.level} xp={character.xp} isEditing={isEditing} onUpdate={handleUpdate} isDead={character.isDead} /></Section>
                    ...
                    <Section><NotesTextarea placeholder='Anote informações da sessão, nomes de NPCs, pistas importantes...' value={character.notes || ''} onChange={(e) => handleUpdate({ notes: e.target.value })} disabled={character.isDead} /></Section>
                </LeftColumn>
