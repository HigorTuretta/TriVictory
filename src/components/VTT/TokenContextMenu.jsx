...
const ResourceControl = ({ label, resourceKey, value, max, color, onAction, editable }) => {
    ...
    return (
        <ResourceBar>
            ...
            {editable && (
                 <ResourceControls>
                    <button onClick={() => handleQuickChange(-1)} disabled={value <= 0}><FaMinus size={12}/></button>
                    <button onClick={() => handleQuickChange(1)} disabled={value >= max}><FaPlus size={12}/></button>
                    <button onClick={() => onAction('fillResource', { resource: resourceKey })} title={`Encher ${label}`}><FaArrowUp size={12}/></button>
                </ResourceControls>
            )}
        </ResourceBar>
    );
